...
int main() {
	...
	for (int i = 1; i < 11; ++i)
	{
        ...
        float current_avg_time = (clock() - start - total_wait_time) / episode;

        std::cout << "Average time : " << current_avg_time << "ms. average fps : " << int(1000 / current_avg_time) << std::endl;

		if (WAIT_TO_SEE)
		{
            wait = clock();
			cv::waitKey();
            total_wait_time += clock() - wait;
		}
		else
		{
			cv::waitKey(1);
