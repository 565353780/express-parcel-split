...
void FrameToBlob(const cv::Mat &frame, InferRequest::Ptr &inferRequest, const std::string &inputName, const bool FLAGS_auto_resize) {
...
int main() {
    ...
    const std::string FLAGS_i = "E:/chLi/express-parcel-split/FilterSocket/bin/01.avi";
    const std::string FLAGS_m = "E:/chLi/express-parcel-split/FilterSocket/models/yolov3/yolov3_17100.xml";
    const std::string FLAGS_d = "CPU";
    const bool FLAGS_pc = false;
    const std::string FLAGS_c = "";
    const std::string FLAGS_l = "";
    const bool FLAGS_r = false;
    const double FLAGS_t = 0.5;
    const double FLAGS_iou_t = 0.4;
    ...
    try {
        ...
        while (true) {
            ...
            if (isAsyncMode) {
                if (isModeChanged) {
                    FrameToBlob(frame, async_infer_request_curr, inputName, FLAGS_auto_resize);
                }
                if (!isLastFrame) {
                    FrameToBlob(next_frame, async_infer_request_next, inputName, FLAGS_auto_resize);
                }
